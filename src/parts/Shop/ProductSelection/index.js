...
export default function ProductSelection(props) {
   ...
   const [selectedMetaList, setSelectedMetaList] = useState({})
   ...
   const fnGetProducts = useCallback(
   ...
   const fnHandleScroll = useCallback(() => {
      const scrollOnEndge =
   ...
   const fnGetCategories = useCallback(() => {
   ...
   const fnToggleSelectVariant = useCallback(
      (product) => {
   ...
   const fnToggleSelectProduct = useCallback(
      (product) => {
   ...
   const fnSelectProduct = useCallback(
      (product, productVariant) => {
         const addProduct = {
            product_id: product.id,
            name: product.name,
            image:
               !!productVariant && !!productVariant.variation_image
                  ? productVariant.variation_image
                  : product.main_image,
            quantity: product.orderQuantity || 1,
            price:
               product.sale_enabled && !productVariant
                  ? product.sale_price
            ...
            meta: {
               ...
               salePercentage:
                  product.sale_enabled && !productVariant
                     ...
                     : undefined,
               variation_options:
                  product?.variation && product?.variation?.is_multivariation
   ...
   const fnChangeRangeProduct = useCallback(
      (event) => {
         ...
         if (newQuantity) {
            const product = Object.values(PRODUCTS.data)
               .filter(
                  (product) =>
                     `${product.id}` === `${event.target.name}`.split('_')[0]
               )
               .map((product) => ({
                  ...product,
                  quantity: newQuantity,
